...
def export_mesh(obj, bones):
    ...
    for f_index, f in enumerate(obj_mesh.polygons):
        f_v = [
            (vi, obj_mesh.vertices[v_idx], l_idx)
    ...
    if bones is not None:
        ...
        for v in obj_mesh.vertices:
            ...
            weight_list = [
                (name, round(e * normalization, 4)) for name, e in weight_list
            ]
...
def export_armature(obj):
    def export_bones(b, list, dict):
        ...
        dict["children"] = [
            export_bones(child, list, OrderedDict()) for child in b.children
        ]
        return dict
        ...
        for t in timelines:
            ...
            for b in bones:
                ...
                if t in dope_sheet[b.name]["timestamp"] or t == 0 or t == timelines[-1]:
                    ...
                    if b.parent is not None:
                        parent_pose_invert = obj.pose.bones[
                            b.parent.name
...
def save(context, **kwargs):
    ...
    if armature_obj is not None:
        ...
        animation_result = export_animation(
            armature_obj, armature_result["joints"].value
        )

    if mesh_obj is not None and export_msh:
        mesh_result = export_mesh(
            mesh_obj,
            armature_result["joints"].value if armature_obj is not None else None,
        )
